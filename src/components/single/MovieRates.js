import React, { useEffect } from 'react'
import Title from '../Title'
import { BsBookmarkStarFill } from 'react-icons/bs'
import { Select, Message } from '../UsedInput'
import { useState } from 'react'
import Rating from '../Stars'
import Empty from '../Notifications/Empty'
import { useDispatch, useSelector } from 'react-redux'
import { yupResolver } from '@hookform/resolvers/yup'
import { loginAction } from '../../Redux/Actions/userAction'
import { LoginValidation } from '../Validation/UserValidation'
import { useForm } from 'react-hook-form'
import { ReviewValidation } from '../Validation/MovieValidation'
import { toast } from 'react-hot-toast'
import { InlineError } from '../Notifications/Error'
import { Link } from 'react-router-dom'
import { movieReviewAction } from '../../Redux/Actions/MovieAction'

const Ratings = [
  {
    title: "0-poor",
    value: 0
  },
  {
    title: "1-Fair",
    value: 1
  },
  {
    title: "2-Good",
    value: 2
  },
  {
    title: "3-Very Good",
    value: 3
  },
  {
    title: "4-Excellent",
    value: 4
  },
  {
    title: "5-Amazing",
    value: 5
  }
];

function MovieRates({ movie }) {
  
  const dispatch = useDispatch();

  const { isLoading, isError } = useSelector((state) => state.createReview)
  const { userInfo } = useSelector((state) => state.userLogin)


  const {
    register,
    handleSubmit,
    watch,
    formState: { errors },
} = useForm({
    resolver: yupResolver(ReviewValidation)
})

//on Submit
const onSubmit = (data) => {
    dispatch(movieReviewAction({
      id: movie?._id,
      review: data
    }))
}

useEffect(() => {
  if(isError){
    toast.error(isError);
    dispatch({type: "MOVIE_REVIEW_RESET"})
  }
})

  return (
    <div className='my-12'>
      <Title title="Reviews" Icon={BsBookmarkStarFill} />
      <div className='mt-10 xl:grid flex flex-col justify-center items-center grid-cols-5 gap-12 bg-dry xs:p-10 py-10  px-2 sm:p-20 rounded'>
        {/* Write Review */}
        <form onSubmit={handleSubmit(onSubmit)} className='xl:col-span-2 w-full flex flex-col gap-8'>
          <h3 className='text-xl text-text font-semibold'>Review "{movie?.name}"</h3>
          <p className='text-sm leading-7 font-medium text-border'>
            Write a review for this Movie. It will be posted on this page.
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam,
          </p>
          <div className='text-sm w-full'>
            <Select label="Select Rating"
              options={Ratings}
              register={{...register("rate")}}
              name="rate"
               />
            <div className='flex mt-4 text-lg gap-2 text-star'>
              <Rating value={watch("rate", false)}/>
            </div>
          </div>
          {
            errors.rate && <InlineError text={errors.rate.message}/>
          }
          {/* Message*/}
          <div className='w-full'>
          <Message name="comment" register={{...register("comment")}} label="Message" placeholder="Make it short and sweet....." />
          {
    
            errors.comment && <InlineError text={errors.comment.message}/>
          }
          </div>
          {/* Submit */}
          {userInfo ? (
            <button disabled={isLoading} type='submit' className='bg-subMain text-white py-3 w-full flex flex-col justify-center items-center rounded'>
              {isLoading ? "Loading..." : "Submit Review"}
          </button>
          ) : (
            <Link to="/login" className='bg-main border border-subMain text-white py-3 w-full flex flex-col justify-center items-center rounded'>
            Login to Review
            </Link>
          )
            }
          
        </form>
        {/*Reviwer */}
        <div className='col-span-3 flex w-full flex-col gap-6'>
          <h3 className='text-xl text-text font-semibold'>Reviews {movie?.numberOfReviews}</h3>
          <div className='w-full flex flex-col bg-main gap-6 rounded-lg md:p-12 p-6 h-header overflow-y-scroll'>
            {
              movie?.reviews?.length > 0 ? movie?.reviews?.map((review) => (
                <div key={review?._id} className='md:grid flex flex-col w-full grid-cols-12 gap-6 bg-dry p-4 border border-gray-800 rounded-lg'>
                  {console.log(movie.reviews)}
                  <div className='col-span-2 bg-main hidden md:block'>
                    <img src={review?.userImage ? `${review.userImage}` : '/images/user2.png'} 
                     alt={review.userName}
                      className='w-full h-24 object-cover rounded-lg' />
                  </div>
                  <div className='col-span-7 flex flex-col gap-2'>
                    <h2>{review?.userName}</h2>
                    <p className='text-sm leading-6 font-medium text-text'>
                      {review?.comment}
                    </p>
                  </div>
                  {/*rate */}
                  <div className='col-span-3 flex flex-row justify-center items-center border-l border-border text-xs gap-1 text-star'>
                    <Rating value={review?.rating} />
                  </div>
                </div>
              )) : (
               <Empty message = {`Be first to rate "${movie?.name}"`}/>
              )
            }
          </div>

        </div>
      </div>
    </div>
  )
}

export default MovieRates